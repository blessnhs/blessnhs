        ...
        BYTE bAffinity )
{
    ...
    //--------------------------------------------------------------------------
    // 메모리 할당
    pbTempFileBuffer = ( BYTE* ) AllocateMemory( dwFileSize );
...
//    pstFile = fopen( pcFileName, "r" );
    if( ( pstFile != NULL ) && 
        ( fl_fread( pbTempFileBuffer, 1, dwFileSize, pstFile ) == dwFileSize ) )
    {
    	fl_fclose( pstFile );
    ...
    //--------------------------------------------------------------------------
    if( LoadProgramAndRelocation( pbTempFileBuffer, &qwApplicationMemory, 
            &qwMemorySize, &qwEntryPointAddress ) == FALSE )
    {
        Printf( "%s file is invalid application file or loading fail\n", 
                 pcFileName );
        FreeMemory( pbTempFileBuffer );
        return TASK_INVALIDID;
    }
    ...
    //--------------------------------------------------------------------------
    // 유저 레벨 응용프로그램 태스크 생성
    pstTask = CreateTask( TASK_FLAGS_USERLEVEL | TASK_FLAGS_PROCESS, 
    ...
    // 인자 문자열 저장
    AddArgumentStringToTask( pstTask, pcArgumentString );
 ...
 */
static BOOL LoadProgramAndRelocation( BYTE* pbFileBuffer, 
        ...
        QWORD* pqwEntryPointAddress )
{
    ...
    for( i = 1 ; i < pstELFHeader->e_shnum ; i++ )
    {
        ...
        if( pstSectionHeader[ i ].sh_type == SHT_NOBITS)
        {
            ...
            MemSet( pstSectionHeader[ i ].sh_addr, 0, pstSectionHeader[ i ].sh_size );
        }
        else
        {
            // 파일 버퍼의 내용을 응용프로그램에게 할당된 메모리로 복사
            MemCpy( pstSectionHeader[ i ].sh_addr, 
                    pbFileBuffer + pstSectionHeader[ i ].sh_offset,
                    pstSectionHeader[ i ].sh_size );
        }        
        Printf( "Section [%x] Virtual Address [%q] File Address [%q] Size [%q]\n",
                i,
    ...
    // 재배치를 수행
    //--------------------------------------------------------------------------
    if( Relocation( pbFileBuffer ) == FALSE )
    {
        Printf( "Relocation fail\n" );
        return FALSE;
    }
    else
    {
...
*/
static BOOL Relocation( BYTE* pbFileBuffer )
{
    Elf64_Ehdr* pstELFHeader;
    Elf64_Shdr* pstSectionHeader;
    int i;
    ...
    for( i = 1 ; i < pstELFHeader->e_shnum ; i++ )
    {
        ...
        for( j = 0 ; j < pstSectionHeader[ i ].sh_size ; )
        {
            ...
            // 공통 타입 심볼(Common Type)의 경우는 지원하지 않으므로 오류를 표시하고 종료
                ...
                SHN_COMMON )
            {
                Printf( "Common symbol is not supported\n" );
                return FALSE;
            }
            ...
            default:
                Printf( "Unsupported relocation type [%X]\n", 
            ...
            default:
                Printf( "Relocation error. Relocation byte size is [%d]byte\n", 
                         iNumberOfBytes );
                return FALSE;
                break;
            }
 ...
 *  태스크에 인자 문자열 저장
 */
static void AddArgumentStringToTask( TCB* pstTask, const char* pcArgumentString )
{
    int iLength;
    int iAlignedLength;
    ...
    MemCpy( ( void* ) qwNewRSPAddress, pcArgumentString, iLength );
        ...
        QWORD qwExitFunction )
{
    ...
    // 유저 레벨 응용프로그램 태스크 생성
    pstTask = CreateTask( TASK_FLAGS_USERLEVEL | TASK_FLAGS_THREAD, NULL, 0, 
    ...
    // 종료될 때 호출되는 EndTask() 함수를 전달받은 함수로 대체
