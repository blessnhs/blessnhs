...
static void DrawDigitalClock( QWORD qwWindowID )
{
    ...
    // 시계 영역의 테두리를 표시
    SetRectangleData( stWindowArea.iX2 - APPLICATIONPANEL_CLOCKWIDTH - 13, 5,
                       stWindowArea.iX2 - 5, 25, &stUpdateArea );    
    DrawRect( qwWindowID, stUpdateArea.iX1, stUpdateArea.iY1, 
    ...
    UpdateScreenByWindowArea( qwWindowID, &stUpdateArea );
}
...
static BOOL ProcessApplicationPanelWindowEvent( void )
{
    ...
    while( 1 )
    {
        ...
        // 이벤트 큐에서 이벤트를 수신
        if( ReceiveEventFromWindowQueue( qwApplicationPanelID, &stReceivedEvent ) 
                == FALSE )
        {
            break;
        }
        ...
        case EVENT_MOUSE_LBUTTONDOWN:
            ...
            if( gs_stApplicationPanelData.bApplicationWindowVisible == FALSE )
            {
                ...
                MoveWindowToTop( gs_stApplicationPanelData.qwApplicationListID );
                ShowWindow( gs_stApplicationPanelData.qwApplicationListID, TRUE );
                // 플래그는 화면에 표시된 것으로 설정
            ...
            else
            {
                ...
                DrawButton( qwApplicationPanelID, 
                         &( gs_stApplicationPanelData.stButtonArea ), 
                ...
                // 애플리케이션 리스트 윈도우를 숨김
                ShowWindow( qwApplicationListID, FALSE );
...
static void DrawApplicationListItem( int iIndex, BOOL bMouseOver )
{
    ...
    // 리스트 아이템의 내부를 채움
    DrawRect( qwWindowID, stItemArea.iX1 + 1, stItemArea.iY1 + 1, 
               stItemArea.iX2 - 1, stItemArea.iY2 - 1, stColor, TRUE );
    
    // GUI 태스크의 이름을 표시
    DrawText( qwWindowID, stItemArea.iX1 + 10, stItemArea.iY1 + 2, 
    ...
    // 업데이트된 아이템을 화면에 업데이트
    UpdateScreenByWindowArea( qwWindowID, &stItemArea );
}
...
static BOOL ProcessApplicationListWindowEvent( void )
{
    ...
    {
        // 이벤트 큐에서 이벤트를 수신
        if( ReceiveEventFromWindowQueue( qwApplicationListID, &stReceivedEvent ) 
                == FALSE )
        {
            break;
        }
        ...
        case EVENT_MOUSE_LBUTTONDOWN:
            ...
            SetMouseEvent( qwApplicationPanelID, EVENT_MOUSE_LBUTTONDOWN,
                    ...
                    NULL, &stEvent );
            SendEventToWindow( qwApplicationPanelID, &stEvent );            
            break;
