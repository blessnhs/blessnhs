...
int usb_diskio_read (uint32 sector, uint8 *buffer, uint32 sector_count)
{
   ...
   if(disk == 0)
   	...
   	int rst = usb_readSectors(sector,buffer,sector_count,disk);
...
int usb_diskio_write(uint32 sector, uint8 *buffer, uint32 sector_count)
{
   ...
   int rst = usb_writeSectors(sector,buffer,sector_count,disk);
   if(CE_GOOD == rst)
   {
   		return 1;
   }
        ...
        if (fatfs_sector_reader(fs, dirls->cluster, dirls->sector, 0))
        {
            ...
            for (item = dirls->offset; item < FAT_DIR_ENTRIES_PER_SECTOR; item++)
            {
                ...
                else if (fatfs_entry_lfn_exists(&lfn, directoryEntry) )
                {
                    // Get text
                    long_filename = fatfs_lfn_cache_get(&lfn);
                    strcpy(entry->filename, long_filename/*, FATFS_MAX_LONG_FILENAME-1*/);
