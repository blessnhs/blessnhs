...
{
	memset(m_szConnect, 0, MAX_CONNECT);
	if(is_ipconnect) swprintf_s(m_szConnect, 128, _T("DRIVER=SQL Server;Server=%s;UID=%s;PWD=%s;"), szDSN, szUID, szPWD);
	else _stprintf(m_szConnect, TEXT("DSN={%s};UID=%s;PWD=%s"), szDSN, szUID, szPWD);
	return Open();
}
