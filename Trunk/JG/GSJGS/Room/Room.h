...
public:
	...
	void SendToAll(WORD MainId,BYTE *Data,WORD Length)
	{
		...
		while(iter != m_PlayerMap.end())
		{
			GSCLIENT *pSession = JGSERVER.GetClient(iter->second->GetPair());
...
protected:
	...
	IGameRule::IGameRule			*pGameRule;
};

typedef boost::shared_ptr<GSFrontServer::Room::Room> RoomPtr;

}	}


#define ROOM		GSFrontServer::Room::Room 
#define ROOM_PTR	GSFrontServer::Room::RoomPtr 
